		...
		if( map.getHitInfosFromRay( barrelPos, -aimVector.Angle(), BULLET_RANGE + rangeOffset, this, @hitInfos ) )
			for (uint i = 0; i < hitInfos.length; i++)
			{
				...
				if ( !b.hasTag( "booty" ) && (bColor > 0 || !isBlock) )
				{
					...
					{
						if ( Block::isSolid(blockType) || ( b.getTeamNum() != teamNum && ( blockType == Block::SHIPCORE || b.hasTag("weapon") || b.hasTag("rocket") || blockType == Block::BOMB ) ) )//hit these and die
							killed = true;
...
void hitEffects( CBlob@ hitBlob, Vec2f worldPoint )
{
	...
	else	if (Block::isSolid(blockType) || blockType == Block::SHIPCORE || hitBlob.hasTag("weapon") 
					|| blockType == Block::PLATFORM || blockType == Block::SEAT || blockType == Block::BOMB)
	{
		sparks(worldPoint, 4);
...
void damageBooty( CPlayer@ attacker, CBlob@ attackerBlob, CBlob@ victim )
{
	if ( victim.getName() == "block" )
	{
		...
		if ( victimIsle !is null && victimIsle.blocks.length > 3
			...
			&& ( victim.hasTag("propeller") || victim.hasTag("weapon") || Block::isBomb( blockType ) || blockType == Block::SEAT )
			)
		{
