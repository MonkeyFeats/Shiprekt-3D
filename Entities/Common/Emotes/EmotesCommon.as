...
void LoadEmotes(CRules@ this, ConfigFile@ cfg)
{
	if (cfg is null)
	{
		warn("missing emote entries config");
		return;
	}
...
{
	dictionary@ emotes;
	if (getRules().get("emotes", @emotes) && emotes !is null)
	{
		return;
	}

	ConfigFile@ cfg = loadEmoteConfig();
	LoadEmotes(getRules(), cfg);
}
...
Emote@[] getWheelEmotes(CRules@ this, ConfigFile@ cfg)
{
	...
	dictionary@ emotes;
	if (!this.get("emotes", @emotes) || emotes is null)
	{
		return wheelEmotes;
	}
	...
	{
		Emote@ emote;
		if (emotes.get(data[i], @emote) && emote !is null)
		{
...
string read_emote(ConfigFile@ cfg, const dictionary &in emotes, CPlayer@ player, const string &in name, const string &in default_value)
{
	...
	if (attempt != "")
	{
		...
		for (int i = 0; i < check_str.length; i++)
		{
			...
			else if (attempt.substr(attempt.size() - 1, 1) == check)
			{
...
bool isMouseOverEmote(CSpriteLayer@ emote)
{
	...
	return mousePos.x >= tl.x && mousePos.y >= tl.y && mousePos.x < br.x && mousePos.y < br.y;
}
...
string[] readEmoteBindings(CPlayer@ player)
{
	...
	{
		string[] defaults = {
			"attn", "smile", "frown", "mad", "laugh", "wat", "troll", "disappoint", "ladder",
			"flex", "down", "smug", "left", "okhand", "right", "thumbsup", "up", "thumbsdown"
		};
		return defaults;
	}
...
bool canUseEmote(CPlayer@ player, Emote@ emote)
{
	...
	string[] excluded = {"dots", "pickup"};
	if (excluded.find(emote.token) > -1) return false;
	if (player !is null && player.isDev()) return true;

	bool patreonEmote = emote.pack.token == "patreon";
	bool patron = player !is null && player.getSupportTier() != SUPPORT_TIER_NONE;

	return !patreonEmote || patron;
}
